...
public class ArduinoReciever : MonoBehaviour
{
    ...
    public UnityEngine.Events.UnityEvent unityEvent = new UnityEngine.Events.UnityEvent();
    ...
    void Start()
    {
        ...
        // Use this PlayerPrefs.GetString("port_name", "/dev/cu.usbserial-AK06RGGT"),

        serialPort = new SerialPort("/dev/cu.usbserial-AK06RGGT", PlayerPrefs.GetInt("port_baud_rate", 57600));
        ...
        // thread attempt
        // Thread thr = new Thread(new ThreadStart(ReceiveInformation));
        // thr.Start();
    ...
    void Update() {
        secondsSinceLastEstTrans += Time.deltaTime;
        if (secondsSinceLastEstTrans > transmissionDelay) {
            secondsSinceLastEstTrans = 0;
            estimatedTransmissions++;
        }

        if (pieChart != null)
            pieChart.ratio = ((float)recievedTransmissions / (float)estimatedTransmissions) * Mathf.PI * 2;
        ...
        debuglabel.text += System.Environment.NewLine;// +  acc_x.ToString();
            //BitConverter.ToString(BitConverter.GetBytes(acc_x)).Replace("-", ""); // + " " + acc_y.ToString() + " " + acc_z.ToString();


        dateTimeAtRecieve = DateTime.Now;

        recievedTransmissions++;
        unityEvent.Invoke();
